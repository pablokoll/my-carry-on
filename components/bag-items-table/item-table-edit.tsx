        ...
        >
          ...
          <td style={{ ...td, textAlign: "center" }}>
            {hasSubs ? (
              ...
              >
                {subs.reduce((a, s) => a + s.quantity, 0)}
              </div>
            ) : (
              <input
                type="number"
          ...
          <td style={td}>
            <select
              value={
                draft.category_id !== null ? String(draft.category_id) : ""
              }
              onChange={(e) =>
                updateDraft(draft.id, {
        ...
        {expandedSubs && (
          <>
            {subs.map((sub) => (
              <tr
                key={`sub-${sub.id}`}
                style={{ background: color ? color.bg : "transparent" }}
              >
                <td style={td}>
                  <button
                    type="button"
                ...
                <td style={{ ...td, paddingLeft: "20px" }}>
                  <input
                    ...
                    onChange={(e) =>
                      updateSubDraft(draft.id, sub.id, { name: e.target.value })
                    }
                    onKeyDown={(e) => {
                ...
                <td style={{ ...td, textAlign: "center" }}>
                  <input
                    ...
                    value={sub.quantity}
                    onChange={(e) =>
                      updateSubDraft(draft.id, sub.id, {
                        quantity: Number(e.target.value) || 1,
                      })
                    }
    ...
    <>
      <div className="overflow-x-auto">
        ...
        >
          ...
          <tbody>{drafts.map((draft, idx) => renderEditRow(draft, idx))}</tbody>
        </table>
      </div>
