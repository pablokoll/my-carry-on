...
export function useBagItems({
  ...
  const columns: ColumnDef<Item>[] = [
    ...
    },
    {
      ...
      filterFn: (row, _, value) =>
        value === "all" ? true : row.original.category_id === value,
    },
  ];
  ...
  function addRow() {
    ...
    setTimeout(() => {
      const inputs =
        document.querySelectorAll<HTMLInputElement>("[data-row-name]");
  ...
  function removeRow(id: number) {
    if (id > 0) {
      setDrafts((prev) =>
        prev.map((d) => (d.id === id ? { ...d, deleted: true } : d)),
      );
    } else {
  ...
  function addSubRow(itemId: number) {
    ...
    setSubDrafts((prev) => ({
      ...
      [itemId]: [
        ...(prev[itemId] ?? []),
        { id: tid, item_id: itemId, name: "", quantity: 1, packed: false },
      ],
    }));
    setExpandedItems((prev) => {
      const s = new Set(prev);
    ...
    setTimeout(() => {
      const inputs = document.querySelectorAll<HTMLInputElement>(
        `[data-sub-name="${itemId}"]`,
      );
      (inputs[inputs.length - 1] as HTMLInputElement)?.focus();
    }, 30);
  }

  function updateSubDraft(
    itemId: number,
    subId: number,
  ...
  async function handleSave() {
    ...
    drafts.forEach((d) => {
      if (!d.deleted && !d.name.trim()) errors.add(d.id);
    });
    const subErrors = new Set<number>();
    Object.values(subDrafts)
      .flat()
      .forEach((s) => {
        if (!s.deleted && !s.name.trim()) subErrors.add(s.id);
      });
    ...
    try {
      ...
      const toUpdate = drafts
        ...
        .filter((d) => {
          ...
          return (
            orig &&
            (orig.name !== d.name ||
              orig.category_id !== d.category_id ||
              orig.quantity !== d.quantity)
          );
        });
      ...
      // biome-ignore lint/suspicious/useIterableCallbackReturn: forEach for side effects, no return needed
      ...
      for (const [rawItemId, subs] of Object.entries(subDrafts)) {
        ...
        const realItemId =
          draftItemId < 0 ? tempToReal.get(draftItemId) : draftItemId;
        if (!realItemId) continue;
        for (const s of subs) {
          if (s.id < 0 && !s.deleted) {
            subOps.push(
              api.post(`/items/${realItemId}/sub-items`, {
                name: s.name.trim(),
                quantity: s.quantity,
              }),
            );
            ...
            if (
              origSub &&
              (origSub.name !== s.name || origSub.quantity !== s.quantity)
            ) {
              subOps.push(
                api.put(`/sub-items/${s.id}`, {
                  name: s.name.trim(),
                  quantity: s.quantity,
                }),
              );
  ...
  async function togglePacked(item: Item) {
    try {
      ...
      syncItems(
        serverItems.map((i) =>
          i.id === updated.id ? { ...i, packed: updated.packed } : i,
        ),
      );
    } catch {
      /* silent */
          ...
          i.id === itemId
            ? {
                ...
                sub_items: i.sub_items.map((s) =>
                  s.id === sub.id ? { ...s, packed: !s.packed } : s,
                ),
              }
            : i,
        ),
